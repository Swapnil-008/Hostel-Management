...
const Reservation = () => {
  ...
  const handleReserve = async (e) => {
    ...
    try {
      // ✅ Use environment-based API URL
      const roomRes = await axios.get(`${API_BASE_URL}/api/rooms?hostel_type=${hostelType}&building_type=${buildingType}&floor=${floor}&room_number=${roomNumber}`);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const options = {
        key: 'rzp_test_9Q6Lg9fdFP0HHi', // Consider moving to .env
        amount: rent * 100,
        currency: 'INR',
        ...
        handler: async (response) => {
          ...
          if (verifyRes.data.success) {
            // ✅ Use environment-based API URL
            await axios.put(`${API_BASE_URL}/api/reservations/${reservationId}`, {
