...
const RoomAllocation = () => {
  ...
  const timerRef = useRef(null); // Store the timer interval ID
    ...
    try {
      ...
      const roomsWithReservationStatus = await Promise.all(
        res.data.map(async (room) => {
          try {
            const reservationRes = await axios.get(`${API_BASE_URL}/api/rooms/check-reservation/${room.id}`);
      ...
      setError(err.response?.data?.error || 'Failed to load rooms. Please try refreshing the page.');
      setRooms([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleReserve = async (roomId) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE_URL}/api/rooms/reserve`, payload);
        ...
        handler: async (response) => {
          try {
            ...
            if (verifyRes.data.success) {
              const confirmRes = await axios.post(`${API_BASE_URL}/api/rooms/confirm-payment`, {
            ...
            console.error('Payment verification or confirmation failed:', verifyErr);
            setError(
              verifyErr.response?.data?.error ||
                `Payment confirmation failed. Please contact support with your payment ID: ${response.razorpay_payment_id}`
            );
          } finally {
            setLoading(false);
          }
  ...
  const handleCancelReservation = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      await axios.post(`${API_BASE_URL}/api/rooms/cancel-reservation`, {
        reservation_id: reservation.reservation_id,
        user_name: userName,
      });
      clearReservation();
