...
const AdminRoomAllocation = () => {
  ...
  const handleAssignStudent = async () => {
    ...
    try {
      const studentIdNum = parseInt(selectedStudent, 10);
      const student = students.find(s => parseInt(s.id, 10) === studentIdNum);
      if (!student) throw new Error(`Student with id ${selectedStudent} not found`);
      ...
      const updatedRoom = {
        member1_id: selectedMember === 'member1' ? studentIdNum : selectedRoom.member1_id,
        member2_id: selectedMember === 'member2' ? studentIdNum : selectedRoom.member2_id
      };
      await axios.put(`${API_BASE_URL}/api/rooms/update/${selectedRoom.id}`, updatedRoom);

      const roomNumber = `${selectedRoom.hostel_type} ${selectedRoom.building_type || ''} Room ${selectedRoom.room_number}`.trim();
      await axios.put(`${API_BASE_URL}/api/students/${studentIdNum}`, { room_number: roomNumber });
      ...
      setStudents(prev =>
        prev.map(s => (parseInt(s.id, 10) === studentIdNum ? { ...s, room_number: roomNumber } : s))
      );
  ...
  const handleRemoveStudent = async (roomId, member) => {
    try {
      ...
      if (!studentId) throw new Error('No student assigned to this slot');
      ...
      await axios.put(`${API_BASE_URL}/api/rooms/update/${roomId}`, updatedRoom);
      ...
      setStudents(prev =>
        prev.map(s => (parseInt(s.id, 10) === studentIdNum ? { ...s, room_number: null } : s))
      );
  ...
  const hasVacancy = (room) => {
    return room.member1_id == null || room.member2_id == null;
  };
